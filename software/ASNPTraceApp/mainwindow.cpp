...
sTraceDevice *MainWindow::getDeviceByAddr(uint8_t addr)
{
    if (addr==0)
        return NULL;
    for (int i = 0; i<m_TraceResult.devices.size(); ++i){
...
void MainWindow::stopWithError()
{
    finishTrace(false);
    ui->statusBar->showMessage(tr("Stoped by error"));
}
...
void MainWindow::onTimerSendStart()
{
    ...
    m_SerialPort.write((char*)data,6);
    QTimer::singleShot(SEND_DELAY,this,SLOT(onTimerSendStep()));
    m_Tracing = true;
...
void MainWindow::onTimerSendIdentification()
{
    ...
    if (device){
        ...
        m_SerialPort.write((char*)data,7);
    }
    else{
        finishTrace(false);
    }
